...
int get_num_procs() { return sysconf(_SC_NPROCESSORS_ONLN); } // The number of processors currently online (available)
...
void *counting_thread(void *args_vptr) {
    ...
    while (true) {
        ...
        // i.e. only guarantee atomicity and modification order consistency.
        int done_copy =
        ...
        // an error is returned if the absolute time specified by abstime passes 
        // (that is, system time equals or exceeds abstime) before the condition cond 
        // is signaled or broadcasted, or if the absolute time specified 
        // by abstime has already been passed at the time of the call
...
void *work(void *args_vptr) {
    ...
    // Поток, которые вызывает функцию pthread_barrier_wait блокируется в месте вызова,
    // до тех пор, пока количество потоков, вызвавших pthread_barrier_wait не станет равным значению count
    ...
    // В случае вызова функция возвращает значение PTHREAD_BARRIER_SERIAL_THREAD 
    // для произвольного синхронизированного потока, и 0 для всех остальных
    // Если функция pthread_barrier_wait возвращает значение PTHREAD_BARRIER_SERIAL_THREAD -> барьер может быть уничтожен
...
int main(int argc, char *argv[]) {
    ...
    pthread_cond_t done_cv = PTHREAD_COND_INITIALIZER;      // condition variable
    ...
    for (int i = 0; i < loop_size; i++) {
        ...
        // механизм синхронизации, который позволяет приостановить 
        // выполнение потоков у некоторой точки программы до тех пор, 
        // пока все потоки не дойдут до этого места, и только затем продолжить выполнение дальше
        pthread_barrier_t barrier;
        // последний аргумент – число потоков, которые должны вызвать pthread_barrier_wait,
        // чтобы преодолеть (разблокировать) барьер.
